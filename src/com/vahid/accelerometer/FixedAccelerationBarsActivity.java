...
public class FixedAccelerationBarsActivity extends Activity {
	...
	private final class DisplayDetectedSituationTask implements Runnable {
		...
		public void run() {
			...
			tvFinalValue.setText(MathUtil
					.round(mAccelerationMagnitude, 3));
			int progressPercentage = (int) (mAccelerationMagnitude * 5);
			...
			if (mAccelerationMagnitude >= Constants.ACCEL_THRESHOLD) {
				if (mCurrentAccelerationBearing > Constants.DIFF_DEGREE_BRAKE) {
					...
					decelerationMovingAverageTime.pushValue(
							(float) mAccelerationMagnitude, new Date());
				...
				// module-Arduino
				if (Constants.BT_MODULE_EXISTS) {
					writeToBluetoothDevice(mAccelerationMagnitude,
							mAccelSituation);
				}
	...
	private void enableBluetoothDialog() {
		AlertDialog.Builder alertDialog = new AlertDialog.Builder(
				FixedAccelerationBarsActivity.this);
		...
		alertDialog.setNegativeButton("No",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						Toast.makeText(FixedAccelerationBarsActivity.this,
